...
function encryptKey(key, userToken) {
    return `${userToken}:${crypto.createHash('sha256').update(`${key}${userToken}`).digest('hex').substring(0, 7)}`;
}
    ...
    axiosReq
        ...
        .then(async (response) => {
            ...
            if (data.instance_data.user.id != null) {
                let userPictureData = await axiosReq.get(
                    `${apiUrl}/misc/downProfile?key=${userId}&id=${data.instance_data.user.id.split(':')[0]}`
                ); // puxa a foto do usuário do whatsapp
                ...
                const userData = {
                    ...
                    userPhone: data.instance_data.user.id.split(':')[0],
                    userId: data.instance_data.instance_key,
                    userPicture: userPictureData.data.data,
                };
